...
const ReDelivered = ({ employeeId, role }) => {
    ...
    const displayData = orderData
        .filter(order => order.status === 're-deleivery' && (role === 'admin' || order.assignedTo === employeeId))
        .map(order => ({
            ...order,
            totalPrice: order.billDetails.reduce((sum, item) => sum + item.totalPrice, 0),
        }));

    return <DataTable columns={orderColumns} data={displayData} title="Re-Delivered" baseURL="/data/order" />;
};

export default ReDelivered;
