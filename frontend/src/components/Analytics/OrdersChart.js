...
const OrdersCard = ({ employeeId, role }) => {
    ...
    const { previousOrders, currentOrders } = getOrdersComparison(orderData.filter(order => order.status === 'verified'), timeFilter);
    ...
    const filteredOrderData = filterDataByTime(orderData.filter(order => order.status === 'verified'), timeFilter);
    ...
    const totalOrders = filteredOrderData.length;
