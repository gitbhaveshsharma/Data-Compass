...
const ReworkHold = React.memo(({ employeeId, role }) => {
    ...
    const displayData = useMemo(() => {
        ...
        return combinedData
            .filter(order => order.status === 'hold' && (role === 'admin' || order.assignedTo === employeeId))
            .map(order => ({
                ...order,
    ...
    return <DataTable columns={orderColumns} data={displayData} title="Rework Hold" baseURL={baseURL} />;
});

export default ReworkHold;
